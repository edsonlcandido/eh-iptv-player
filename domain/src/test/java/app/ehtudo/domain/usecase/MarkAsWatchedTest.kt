...
class MarkAsWatchedTest {
    ...
    private class FakePlaybackHistoryRepository : PlaybackHistoryRepository {
        ...
        override suspend fun markAsWatched(history: PlaybackHistory): app.ehtudo.domain.model.Result<Unit> {
            lastMarkedHistory = history
            return app.ehtudo.domain.model.Result.success(Unit)
        }
        override suspend fun recordPlayback(history: PlaybackHistory) = app.ehtudo.domain.model.Result.success(Unit)
        override suspend fun updateResumePosition(history: PlaybackHistory) = app.ehtudo.domain.model.Result.success(Unit)
        override suspend fun flushPendingProgress() = app.ehtudo.domain.model.Result.success(Unit)
        override suspend fun removeFromHistory(contentId: Long, contentType: ContentType, providerId: Long) = app.ehtudo.domain.model.Result.success(Unit)
        override suspend fun clearAllHistory() = app.ehtudo.domain.model.Result.success(Unit)
        override suspend fun clearHistoryForProvider(providerId: Long) = app.ehtudo.domain.model.Result.success(Unit)
        override suspend fun clearLiveHistoryForProvider(providerId: Long) = app.ehtudo.domain.model.Result.success(Unit)
    }
}
