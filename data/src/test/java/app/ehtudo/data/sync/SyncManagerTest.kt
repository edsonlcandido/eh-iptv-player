...
import app.ehtudo.data.local.dao.ChannelStageCategorySummary
...
import app.ehtudo.data.local.entity.MovieCategoryHydrationEntity
import app.ehtudo.data.local.entity.MovieEntity
import app.ehtudo.data.local.entity.ProviderEntity
import app.ehtudo.data.local.entity.SeriesCategoryHydrationEntity
...
import app.ehtudo.domain.model.ProviderXtreamLiveSyncMode
...
import app.ehtudo.data.remote.stalker.StalkerCategoryRecord
import app.ehtudo.data.remote.stalker.StalkerItemRecord
import app.ehtudo.data.remote.stalker.StalkerPagedItems
import app.ehtudo.data.remote.stalker.StalkerProgramRecord
import app.ehtudo.data.remote.stalker.StalkerProviderProfile
import app.ehtudo.data.remote.stalker.StalkerSession
import app.ehtudo.data.remote.stalker.StalkerApiService
import app.ehtudo.data.remote.stalker.StalkerAdvancedOptions
import app.ehtudo.data.remote.stalker.StalkerAdvancedOptionsCodec
import app.ehtudo.data.remote.stalker.StalkerDeviceProfile
...
class SyncManagerTest {
    ...
    private class FakeSyncMetadataRepository : app.ehtudo.domain.repository.SyncMetadataRepository {
    ...
    fun setup() {
        ...
        runBlocking {
            ...
            org.mockito.kotlin.whenever(epgSourceRepo.resolveForProvider(any(), any()))
                .thenReturn(app.ehtudo.domain.model.EpgResolutionSummary())
        }
    }
    ...
    fun `sync_xtream_staged_live_commit_preserves_hidden_categories_and_channels`() = runTest {
        ...
        assertThat(result.isSuccess).isTrue()
        val insertedCategories = argumentCaptor<List<app.ehtudo.data.local.entity.CategoryImportStageEntity>>()
        val insertedChannels = argumentCaptor<List<app.ehtudo.data.local.entity.ChannelImportStageEntity>>()
    ...
    fun `retrySection_movies_queues_xtream_index_categories_only`() = runTest {
        ...
        assertThat(metadata?.movieSyncMode).isEqualTo(app.ehtudo.domain.model.VodSyncMode.UNKNOWN)
        assertThat(xtreamBackend.requestedActions).contains("get_vod_categories")
        assertThat(xtreamBackend.requestedActions).doesNotContain("get_vod_streams")
        val jobs = argumentCaptor<XtreamIndexJobEntity>()
        verify(xtreamIndexJobDao, atLeastOnce()).upsert(jobs.capture())
    ...
    fun sync_stalker_persists_only_vod_and_series_categories_during_initial_sync() = runTest {
        ...
        assertThat(result).isInstanceOf(Result.Success::class.java)
        val metadata = syncMetadataRepo.getMetadata(1L)
        assertThat(metadata?.movieSyncMode).isEqualTo(app.ehtudo.domain.model.VodSyncMode.PAGED)
    ...
    fun sync_stalker_persists_wildcard_vod_and_series_categories_when_portal_category_lists_are_empty() = runTest {
        ...
        assertThat(result).isInstanceOf(Result.Success::class.java)
        val categoryStagesCaptor = argumentCaptor<List<app.ehtudo.data.local.entity.CategoryImportStageEntity>>()
    ...
    fun sync_stalker_recovers_when_live_categories_fail_but_bulk_channels_work() = runTest {
        ...
        val categoryStagesCaptor = argumentCaptor<List<app.ehtudo.data.local.entity.CategoryImportStageEntity>>()
        verify(catalogSyncDao, atLeastOnce()).insertCategoryStages(categoryStagesCaptor.capture())
        assertThat(categoryStagesCaptor.allValues.flatten().any { it.type == app.ehtudo.domain.model.ContentType.LIVE && it.name == "News" }).isTrue()
    ...
    fun sync_stalker_recovers_when_bulk_live_streaming_fails_using_category_fetches() = runTest {
        ...
        verify(catalogSyncDao, atLeastOnce()).insertCategoryStages(categoryStagesCaptor.capture())
        assertThat(categoryStagesCaptor.allValues.flatten().any { it.type == app.ehtudo.domain.model.ContentType.LIVE && it.name == "News" }).isTrue()
        assertThat(categoryStagesCaptor.allValues.flatten().any { it.type == app.ehtudo.domain.model.ContentType.LIVE && it.name == "Sports" }).isTrue()
        verify(stalkerApiService).streamLiveStreams(any(), any(), any())
 ...
 */
private suspend fun app.ehtudo.data.remote.stalker.StalkerApiService.stubStreamBulkEpg(
    programs: List<app.ehtudo.data.remote.stalker.StalkerProgramRecord>,
    periodHours: Int = 6
) {
    org.mockito.kotlin.whenever(
        streamBulkEpg(
        ...
        val cb = invocation.arguments[3] as suspend (app.ehtudo.data.remote.stalker.StalkerProgramRecord) -> Unit
        programs.forEach { cb(it) }
        app.ehtudo.domain.model.Result.success(programs.size)
    }
}

/** Stubs the streaming live-channel path. Emits [items] through the captured callback. */
private suspend fun app.ehtudo.data.remote.stalker.StalkerApiService.stubStreamLiveStreams(
    items: List<app.ehtudo.data.remote.stalker.StalkerItemRecord>
) {
    org.mockito.kotlin.whenever(
        streamLiveStreams(
            org.mockito.kotlin.any(),
        ...
        @Suppress("UNCHECKED_CAST")
        val cb = invocation.arguments[2] as suspend (app.ehtudo.data.remote.stalker.StalkerItemRecord) -> Unit
        items.forEach { cb(it) }
        app.ehtudo.domain.model.Result.success(items.size)
    }
}

/** Stubs the streaming live-channel path to return an error result without emitting items. */
private suspend fun app.ehtudo.data.remote.stalker.StalkerApiService.stubStreamLiveStreamsError(message: String) {
    org.mockito.kotlin.whenever(
        streamLiveStreams(
            org.mockito.kotlin.any(),
            org.mockito.kotlin.any(),
    ...
    ).doSuspendableAnswer { app.ehtudo.domain.model.Result.error(message) }
}

/** Counterpart to [stubStreamBulkEpg] for the per-channel streamed EPG path. */
private suspend fun app.ehtudo.data.remote.stalker.StalkerApiService.stubStreamEpg(
    channelId: String,
    programs: List<app.ehtudo.data.remote.stalker.StalkerProgramRecord>,
    periodHours: Int = 6
) {
    org.mockito.kotlin.whenever(
        ...
        @Suppress("UNCHECKED_CAST")
        val cb = invocation.arguments[4] as suspend (app.ehtudo.data.remote.stalker.StalkerProgramRecord) -> Unit
        programs.forEach { cb(it) }
        app.ehtudo.domain.model.Result.success(programs.size)
    }
}
