...
class StalkerProviderTest {
    ...
    fun buildCatchUpUrls_returns_internal_archive_candidates_for_live_token() = runTest {
        val provider = StalkerProvider(
            ...
            portalUrl = "https://portal.example.com/c/",
            macAddress = "00:1A:79:12:34:56",
            playbackBackendHint = app.ehtudo.domain.model.StalkerPlaybackBackendHint.PLAY_LIVE,
            deviceProfile = "MAG250",
            timezone = "UTC",
            locale = "en"
        )
    ...
    fun resolvePlaybackInfo_omits_authorization_for_play_live_temp_links() = runTest {
        val provider = StalkerProvider(
            ...
            portalUrl = "https://portal.example.com/c/",
            macAddress = "00:1A:79:12:34:56",
            playbackBackendHint = app.ehtudo.domain.model.StalkerPlaybackBackendHint.TEMP_LINK_STRICT,
            cookieModeHint = app.ehtudo.domain.model.StalkerCookieMode.CREATE_LINK,
        ...
        assertThat(success.data.cookieMode).isNotEqualTo(app.ehtudo.domain.model.StalkerCookieMode.NONE)
    }
    ...
    fun authenticate_persists_effective_learned_mag_identity() = runTest {
        val provider = StalkerProvider(
            ...
            api = FakeStalkerApiService(
                profile = StalkerProviderProfile(
                    accountName = "Room",
                    effectiveAuthMode = app.ehtudo.domain.model.StalkerAuthMode.MAC_ONLY,
                    portalProfile = app.ehtudo.domain.model.StalkerPortalProfile.MODULE_GATED,
                    portalFingerprint = app.ehtudo.domain.model.StalkerPortalFingerprint.MODULE_GATED,
                    magPreset = app.ehtudo.domain.model.StalkerMagPreset.MINISTRA_MODERN,
                    bootstrapRecipe = app.ehtudo.domain.model.StalkerBootstrapRecipe.MODULE_GATED,
                    fingerprintEvidence = StalkerFingerprintEvidence(
                        endpointPreference = app.ehtudo.domain.model.StalkerEndpointPreference.PORTAL,
                        cookieMode = app.ehtudo.domain.model.StalkerCookieMode.BOTH,
                        playbackBackendHint = app.ehtudo.domain.model.StalkerPlaybackBackendHint.TEMP_LINK_STRICT
                    )
                )
            ),
        ...
        assertThat(result).isInstanceOf(Result.Success::class.java)
        val success = result as Result.Success
        assertThat(success.data.stalkerMagPreset).isEqualTo(app.ehtudo.domain.model.StalkerMagPreset.MINISTRA_MODERN)
