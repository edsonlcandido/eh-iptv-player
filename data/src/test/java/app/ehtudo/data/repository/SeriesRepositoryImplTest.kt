...
import app.ehtudo.data.local.entity.EpisodeBrowseEntity
import app.ehtudo.data.local.entity.EpisodeEntity
import app.ehtudo.data.local.entity.SeriesEntity
import app.ehtudo.data.local.entity.SeriesBrowseEntity
...
import app.ehtudo.data.remote.dto.XtreamSeason
import app.ehtudo.data.remote.dto.XtreamSeriesInfoResponse
import app.ehtudo.data.remote.stalker.StalkerCategoryRecord
import app.ehtudo.data.remote.stalker.StalkerEpisodeRecord
...
import app.ehtudo.data.remote.stalker.StalkerSeasonRecord
import app.ehtudo.data.remote.stalker.StalkerSeriesDetails
...
import app.ehtudo.domain.model.LibraryBrowseQuery
import app.ehtudo.domain.model.LibraryFilterBy
...
import app.ehtudo.domain.model.SeriesDetailPresentationHint
import app.ehtudo.domain.model.VodDuplicateConfidence
import app.ehtudo.domain.model.VodDuplicateHandlingMode
import app.ehtudo.domain.model.VodSeriesVariant
import app.ehtudo.domain.model.VodVariantObservation
...
class SeriesRepositoryImplTest {
    ...
    fun `stalker series preview loads only first page`() = runTest {
        ...
        whenever(categoryDao.getByProviderAndType(7L, ContentType.SERIES.name)).thenReturn(
            flowOf(listOf(app.ehtudo.data.local.entity.CategoryEntity(providerId = 7L, categoryId = 77L, name = "Drama", type = ContentType.SERIES)))
        )
        whenever(providerDao.getById(7L)).thenReturn(
            ProviderEntity(
                id = 7L,
    ...
    fun `getSeriesDetails falls back to remote series id lookup`() = runTest {
        ...
        assertThat(result).isInstanceOf(app.ehtudo.domain.model.Result.Success::class.java)
        val series = (result as app.ehtudo.domain.model.Result.Success).data
