...
import app.ehtudo.data.local.entity.FavoriteEntity
import app.ehtudo.data.local.entity.MovieBrowseEntity
import app.ehtudo.data.local.entity.MovieEntity
import app.ehtudo.data.local.entity.PlaybackHistoryLiteEntity
...
class MovieRepositoryImplTest {
    ...
    fun `stalker movie preview loads only first page`() = runTest {
        ...
        whenever(categoryDao.getByProviderAndType(7L, ContentType.MOVIE.name)).thenReturn(
            flowOf(listOf(app.ehtudo.data.local.entity.CategoryEntity(providerId = 7L, categoryId = 42L, name = "Action", type = ContentType.MOVIE)))
        )
        whenever(providerDao.getById(7L)).thenReturn(
            ProviderEntity(
                id = 7L,
