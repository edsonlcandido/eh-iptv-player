...
internal suspend fun reconcileTargetedProviderStatus(
    ...
    provider: app.ehtudo.data.local.entity.ProviderEntity,
    result: app.ehtudo.domain.model.Result<Unit>,
    ...
    when (result) {
        ...
        is app.ehtudo.domain.model.Result.Loading -> Unit
    }
}
    ...
    private suspend fun syncXtreamProviderIfStale(
        ...
        if (liveStale) {
            when (val liveResult = entryPoint.syncManager().retrySection(
                ...
                is app.ehtudo.domain.model.Result.Error -> return liveResult
                else -> Unit
            }
        }
        if (epgStale) {
            when (val epgResult = entryPoint.syncManager().syncEpg(provider.id, force = false)) {
                is app.ehtudo.domain.model.Result.Error -> return epgResult
                else -> Unit
            }
        }
        if (movieIndexDue) {
