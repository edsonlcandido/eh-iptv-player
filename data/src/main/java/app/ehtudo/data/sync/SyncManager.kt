...
import app.ehtudo.data.local.dao.CatalogSyncDao
import app.ehtudo.data.local.dao.CategoryDao
import app.ehtudo.data.local.dao.ChannelDao
import app.ehtudo.data.local.dao.EpisodeDao
import app.ehtudo.data.local.dao.MovieCategoryHydrationDao
import app.ehtudo.data.local.dao.MovieDao
import app.ehtudo.data.local.dao.ProgramDao
import app.ehtudo.data.local.dao.ProviderDao
import app.ehtudo.data.local.dao.SeriesCategoryHydrationDao
import app.ehtudo.data.local.dao.SeriesDao
import app.ehtudo.data.local.dao.TmdbIdentityDao
import app.ehtudo.data.local.dao.XtreamContentIndexDao
import app.ehtudo.data.local.dao.XtreamIndexJobDao
import app.ehtudo.data.local.dao.XtreamLiveOnboardingDao
import app.ehtudo.data.local.entity.CategoryEntity
import app.ehtudo.data.local.entity.ChannelEntity
import app.ehtudo.data.local.entity.ChannelGuideSyncEntity
import app.ehtudo.data.local.entity.MovieEntity
import app.ehtudo.data.local.entity.SeriesEntity
import app.ehtudo.data.local.entity.XtreamContentIndexEntity
import app.ehtudo.data.local.entity.XtreamIndexJobEntity
import app.ehtudo.data.local.entity.XtreamLiveOnboardingStateEntity
import app.ehtudo.data.mapper.toDomain
import app.ehtudo.data.mapper.toEntity
import app.ehtudo.data.parser.M3uParser
import app.ehtudo.data.remote.http.buildAppRequestProfile
import app.ehtudo.data.remote.http.toGenericRequestProfile
import app.ehtudo.data.remote.stalker.StalkerApiService
import app.ehtudo.data.remote.stalker.StalkerPlaybackMode
import app.ehtudo.data.remote.stalker.StalkerProvider
import app.ehtudo.data.remote.stalker.StalkerProviderProfile
import app.ehtudo.data.remote.jellyfin.JellyfinProvider
import app.ehtudo.data.remote.stalker.StalkerTrafficCoordinator
import app.ehtudo.data.remote.dto.XtreamCategory
import app.ehtudo.data.remote.dto.XtreamSeriesItem
import app.ehtudo.data.remote.dto.XtreamStream
import app.ehtudo.data.remote.xtream.OkHttpXtreamApiService
import app.ehtudo.data.remote.xtream.XtreamAuthenticationException
import app.ehtudo.data.remote.xtream.XtreamNetworkException
import app.ehtudo.data.remote.xtream.XtreamParsingException
import app.ehtudo.data.remote.xtream.XtreamRequestException
import app.ehtudo.data.remote.xtream.XtreamResponseTooLargeException
import app.ehtudo.data.remote.xtream.XtreamApiService
import app.ehtudo.data.remote.xtream.XtreamProvider
import app.ehtudo.data.remote.xtream.XtreamUrlFactory
import app.ehtudo.data.security.CredentialCrypto
import app.ehtudo.data.util.AdultContentClassifier
import app.ehtudo.data.util.UrlSecurityPolicy
...
import app.ehtudo.domain.model.ProviderEpgSyncMode
...
import app.ehtudo.domain.model.VodSyncMode
import app.ehtudo.domain.repository.EpgRepository
import app.ehtudo.domain.repository.EpgSourceRepository
...
class SyncManager @Inject constructor(
    ...
    private val preferencesRepository: app.ehtudo.data.preferences.PreferencesRepository,
    ...
    ): app.ehtudo.domain.model.Result<Unit> {
        val providerEntity = providerDao.getById(providerId)
            ?: return app.ehtudo.domain.model.Result.error("Provider $providerId not found")
        ...
        return if (providerEntity.type == ProviderType.STALKER_PORTAL) {
            ...
            withProviderLock(providerId) {
                val freshProviderEntity = providerDao.getById(providerId)
                    ?: return@withProviderLock app.ehtudo.domain.model.Result.error("Provider $providerId not found")
    ...
    private suspend fun syncEpgLocked(
        providerEntity: app.ehtudo.data.local.entity.ProviderEntity,
        force: Boolean,
        ...
        if (!force && provider.type == ProviderType.STALKER_PORTAL && hasPendingStalkerCatalogIndex(provider.id)) {
            ...
            return app.ehtudo.domain.model.Result.success(Unit)
        }
        ...
        return try {
            ...
            app.ehtudo.domain.model.Result.success(Unit)
            ...
            app.ehtudo.domain.model.Result.error(safeMessage, e)
        }
    }
    ...
    ): app.ehtudo.domain.model.Result<Unit> = withProviderLock(providerId) lock@{
        try {
            val providerEntity = providerDao.getById(providerId)
                ?: return@lock app.ehtudo.domain.model.Result.error("Provider $providerId not found")
    ...
    suspend fun rebuildXtreamIndex(
        ...
        if (providerEntity.type != ProviderType.XTREAM_CODES) {
            return@lock app.ehtudo.domain.model.Result.error("Index rebuild is only available for Xtream providers")
        }
    ...
    private suspend fun syncXtreamIndexFirst(
        ...
        syncProgressBus.emit(
            app.ehtudo.domain.sync.SyncProgress(
                section = app.ehtudo.domain.sync.Section.VOD,
                current = 0,
                total = 0,
                currentLabel = "",
        ...
        syncProgressBus.emit(
            app.ehtudo.domain.sync.SyncProgress(
                section = app.ehtudo.domain.sync.Section.SERIES,
                current = 0,
                total = 0,
                currentLabel = "",
                itemsIndexed = liveCount
    ...
    suspend fun processQueuedXtreamIndexJobs(
        ...
        val providerEntity = providerDao.getById(providerId)
            ?: return@lock app.ehtudo.domain.model.Result.error("Provider $providerId not found")
        if (providerEntity.type != ProviderType.XTREAM_CODES) {
            return@lock app.ehtudo.domain.model.Result.success(Unit)
        }
        ...
        if (warnings.isNotEmpty()) {
            ...
            app.ehtudo.domain.model.Result.error(warnings.first(), exception)
        } else {
            app.ehtudo.domain.model.Result.success(Unit)
        }
    }
    ...
    private suspend fun processStalkerSummaryIndexSection(
        ...
        data class StalkerCategoryFetchResult(
            ...
            val result: Result<app.ehtudo.data.remote.stalker.StalkerPagedResult<out Any>>
        )
            ...
            when (val result = fetchStalkerWildcardSummaryPageWithRecovery(api, contentType, wildcardCategory.categoryId, nextPage)) {
                is Result.Success -> {
                    ...
                    val anomaly = detectStalkerPageAnomaly(
                        hydration = currentHydration,
                        requestedPage = nextPage,
                        pagedResult = app.ehtudo.data.remote.stalker.StalkerPagedResult(
                            items = visibleItems,
    ...
    private suspend fun markStalkerAttemptStarted(
        ...
        when (contentType) {
            ContentType.MOVIE -> movieCategoryHydrationDao.upsert(
                app.ehtudo.data.local.entity.MovieCategoryHydrationEntity(
                    providerId = providerId,
                    categoryId = categoryId,
            ...
            ContentType.SERIES -> seriesCategoryHydrationDao.upsert(
                app.ehtudo.data.local.entity.SeriesCategoryHydrationEntity(
                    providerId = providerId,
                    categoryId = categoryId,
                    lastHydratedAt = hydration?.lastHydratedAt ?: 0L,
                    itemCount = hydration?.itemCount ?: 0,
    ...
    private suspend fun fetchStalkerSummaryPageWithRecovery(
        ...
        categoryId: Long,
        page: Int
    ): Result<app.ehtudo.data.remote.stalker.StalkerPagedResult<out Any>> {
    ...
    private suspend fun fetchStalkerSummaryFirstPageFallback(
        ...
        contentType: ContentType,
        categoryId: Long
    ): Result<app.ehtudo.data.remote.stalker.StalkerPagedResult<out Any>> = when (contentType) {
        ContentType.MOVIE -> when (val result = api.getVodStreams(categoryId)) {
            is Result.Success -> Result.success(
                app.ehtudo.data.remote.stalker.StalkerPagedResult(
                    items = result.data,
                    page = 1,
                    totalPages = 1,
    ...
    private fun detectStalkerPageAnomaly(
        hydration: StalkerHydrationSnapshot?,
        requestedPage: Int,
        pagedResult: app.ehtudo.data.remote.stalker.StalkerPagedResult<out Any>,
        pageFingerprint: String?
    ): String? {
    ...
    private suspend fun syncStalker(
        ...
        if (force || ContentCachePolicy.shouldRefresh(metadata.lastMovieSuccess, ContentCachePolicy.CATALOG_TTL_MILLIS, now)) {
            ...
            val categories = when (val categoriesResult = api.getVodCategories()) {
                is app.ehtudo.domain.model.Result.Success -> categoriesResult.data
                is app.ehtudo.domain.model.Result.Error -> {
                ...
                is app.ehtudo.domain.model.Result.Loading -> throw IllegalStateException("Unexpected loading state")
            }
            emitCatalogSyncProgress(
                section = Section.VOD,
    ...
    private suspend fun syncJellyfin(
        ...
        try {
            ...
            if (resolvedMovies is app.ehtudo.domain.model.Result.Error) android.util.Log.e("JellyfinSync", "Movies error: ${resolvedMovies.message}")
            if (resolvedSeries is app.ehtudo.domain.model.Result.Error) android.util.Log.e("JellyfinSync", "Series error: ${resolvedSeries.message}")

            if (resolvedMovies is app.ehtudo.domain.model.Result.Error && resolvedSeries is app.ehtudo.domain.model.Result.Error) {
            ...
            val movieEntities = (resolvedMovies as? app.ehtudo.domain.model.Result.Success)?.data.orEmpty()
            val seriesEntities = (resolvedSeries as? app.ehtudo.domain.model.Result.Success)?.data.orEmpty()
            ...
            if (movieEntities.isNotEmpty()) {
                ...
                transactionRunner.inTransaction {
                    categoryDao.replaceAll(provider.id, app.ehtudo.domain.model.ContentType.MOVIE.name, listOf(
                        app.ehtudo.data.local.entity.CategoryEntity(
                            providerId = provider.id, categoryId = 1L, name = "Movies",
                            type = app.ehtudo.domain.model.ContentType.MOVIE
                        )
                    ))
                }
            }
            ...
            if (seriesEntities.isNotEmpty()) {
                ...
                transactionRunner.inTransaction {
                    categoryDao.replaceAll(provider.id, app.ehtudo.domain.model.ContentType.SERIES.name, listOf(
                        app.ehtudo.data.local.entity.CategoryEntity(
                            providerId = provider.id, categoryId = 2L, name = "Series",
                            type = app.ehtudo.domain.model.ContentType.SERIES
                        )
                    ))
                }
            }
    ...
    private suspend fun syncStalkerPortalEpg(
        ...
        val insertBuffer = ArrayList<app.ehtudo.data.local.entity.ProgramEntity>(STALKER_GUIDE_PROGRAM_BATCH_SIZE)
        ...
        runCatching {
            ...
            api.streamBulkEpg(periodHours = 6) { program ->
                ...
                if (result is app.ehtudo.domain.model.Result.Error) {
        ...
        fallbackGuideRequests.forEachIndexed { index, request ->
            ...
            runCatching {
                ...
                if (streamResult is app.ehtudo.domain.model.Result.Error) {
    ...
    private suspend fun syncXtreamLiveCatalog(
        ...
        syncProgressBus.emit(
            app.ehtudo.domain.sync.SyncProgress(
                section = app.ehtudo.domain.sync.Section.LIVE,
                current = 0,
                total = 0,
                currentLabel = "",
                itemsIndexed = 0
    ...
    private suspend fun loadStalkerChannelsByCategory(
        api: StalkerProvider,
        categories: List<app.ehtudo.domain.model.Category>,
        onProgress: ((String) -> Unit)?
    ...
    private suspend fun syncStalkerLiveCatalogStaged(
        ...
        try {
            ...
            when (val streamResult = api.streamLiveStreams { channel ->
                ...
                is app.ehtudo.domain.model.Result.Success -> {
                    flushBatch()
    ...
    private suspend fun stalkerCatalogAccessDiagnostic(
        ...
        val profile = when (val profileResult = api.getAccountProfile()) {
            is app.ehtudo.domain.model.Result.Success -> profileResult.data
            else -> return null
        }
    ...
    private fun <T> requireResult(result: app.ehtudo.domain.model.Result<T>, fallbackMessage: String): T {
        return when (result) {
            is app.ehtudo.domain.model.Result.Success -> result.data
            is app.ehtudo.domain.model.Result.Error -> throw IllegalStateException(result.message.ifBlank { fallbackMessage }, result.exception)
            is app.ehtudo.domain.model.Result.Loading -> throw IllegalStateException("Unexpected loading state")
        }
    }
