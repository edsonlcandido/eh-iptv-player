...
import app.ehtudo.data.local.dao.CombinedM3uProfileDao
import app.ehtudo.data.local.dao.CombinedM3uProfileMemberDao
import app.ehtudo.data.local.dao.ProviderDao
import app.ehtudo.data.local.entity.CombinedM3uProfileEntity
import app.ehtudo.data.local.entity.CombinedM3uProfileMemberEntity
            ...
            if (memberProviders.isEmpty()) {
                ...
                combine(categoryFlows) { arrays ->
                    arrays.toList()
                        .map { it as Triple<*, *, *> }
                        .flatMap { triple ->
                            val member = triple.first as app.ehtudo.domain.model.CombinedM3uProfileMember
                            val categories = triple.second as List<Category>
                            val hiddenCategoryIds = triple.third as Set<Long>
                            categories
                                .filter {
                                    !it.isVirtual &&
                                        it.id != app.ehtudo.domain.repository.ChannelRepository.ALL_CHANNELS_ID &&
