package app.ehtudo.data.manager.recording

import app.ehtudo.data.local.dao.ProviderDao
import app.ehtudo.data.remote.xtream.ResolvedStreamUrl
...
class RecordingSourceResolver @Inject constructor(
    ...
    suspend fun resolveLiveSource(
        ...
        val providerLabel = providerDao.getById(providerId)?.let { provider ->
            when {
                provider.type == app.ehtudo.domain.model.ProviderType.XTREAM_CODES -> "${provider.name} • Xtream"
                provider.type == app.ehtudo.domain.model.ProviderType.M3U -> "${provider.name} • M3U"
                provider.type == app.ehtudo.domain.model.ProviderType.STALKER_PORTAL -> "${provider.name} • Stalker"
                else -> provider.name
            }
        }
