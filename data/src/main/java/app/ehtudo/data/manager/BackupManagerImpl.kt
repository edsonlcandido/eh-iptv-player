package app.ehtudo.data.manager
...
import app.ehtudo.data.local.dao.FavoriteDao
import app.ehtudo.data.local.dao.MovieDao
import app.ehtudo.data.local.dao.PlaybackHistoryDao
import app.ehtudo.data.local.dao.ProviderDao
import app.ehtudo.data.local.dao.RecordingScheduleDao
import app.ehtudo.data.local.dao.VirtualGroupDao
import app.ehtudo.data.local.entity.ProviderEntity
import app.ehtudo.data.local.entity.RecordingScheduleEntity
import app.ehtudo.data.mapper.toDomain
import app.ehtudo.data.mapper.toEntity
import app.ehtudo.data.preferences.ParentalPinBackupData
import app.ehtudo.data.preferences.PreferencesRepository
import app.ehtudo.data.security.CredentialCrypto
import app.ehtudo.domain.manager.BackupData
import app.ehtudo.domain.manager.BackupConflictStrategy
import app.ehtudo.domain.manager.BackupImportPlan
import app.ehtudo.domain.manager.BackupImportResult
import app.ehtudo.domain.manager.BackupManager
import app.ehtudo.domain.manager.BackupPreview
import app.ehtudo.domain.manager.RecordingScheduleImportDisposition
import app.ehtudo.domain.manager.RecordingScheduleImportOutcome
import app.ehtudo.domain.manager.RecordingScheduleImportSummary
import app.ehtudo.domain.manager.ProtectedCategoryBackup
import app.ehtudo.domain.manager.RecordingManager
import app.ehtudo.domain.manager.ScheduledRecordingBackup
...
class BackupManagerImpl @Inject constructor(
    ...
    override suspend fun exportConfig(uriString: String): app.ehtudo.domain.model.Result<Unit> = withContext(Dispatchers.IO) {
        try {
            ...
            } ?: return@withContext app.ehtudo.domain.model.Result.error("Failed to open output stream")

            app.ehtudo.domain.model.Result.success(Unit)
        } catch (e: Exception) {
            app.ehtudo.domain.model.Result.error("Failed to export backup: ${e.message}", e)
        }
    }
    ...
    ): app.ehtudo.domain.model.Result<BackupImportResult> = withContext(Dispatchers.IO) {
        try {
            val backupData = readBackupData(uriString)
                ?: return@withContext app.ehtudo.domain.model.Result.error("Failed to open input stream")

            if (backupData.version > CURRENT_BACKUP_VERSION) {
                return@withContext app.ehtudo.domain.model.Result.error("Unsupported backup version")
            }
            if (backupData.isStructurallyEmpty()) {
                return@withContext app.ehtudo.domain.model.Result.error("Backup file does not contain any importable data")
            }
            if (!verifyChecksum(backupData)) {
                return@withContext app.ehtudo.domain.model.Result.error("Backup file is corrupted (checksum mismatch)")
            }
            ...
            app.ehtudo.domain.model.Result.success(
                BackupImportResult(
            ...
            app.ehtudo.domain.model.Result.error("Failed to import backup: ${e.message}", e)
        }
    }
    ...
    private suspend fun restorePreferences(prefs: Map<String, String>) {
        ...
        prefs["appLandingDestination"]?.takeIf { it.isNotBlank() }?.let { savedDestination ->
            preferencesRepository.setAppLandingDestination(
                app.ehtudo.domain.model.AppLandingDestination.fromStorage(savedDestination)
            )
        }
        ...
        val legacyDecoderMode = prefs["playerDecoderMode"]
            ...
            ?.let { savedMode ->
                app.ehtudo.domain.model.DecoderMode.entries
        ...
        prefs["playerAudioOutputPreference"]?.takeIf { it.isNotBlank() }?.let { savedPreference ->
            val preference = app.ehtudo.domain.model.AudioOutputPreference.entries
        ...
        prefs["playerSurfaceMode"]?.takeIf { it.isNotBlank() }?.let { savedMode ->
            val surfaceMode = app.ehtudo.domain.model.PlayerSurfaceMode.entries
                .firstOrNull { entry -> entry.name == savedMode }
            if (surfaceMode != null) {
        ...
        prefs["playerLiveStreamFormatMode"]?.takeIf { it.isNotBlank() }?.let { savedMode ->
            val formatMode = app.ehtudo.domain.model.LiveStreamFormatMode.entries
                .firstOrNull { entry -> entry.name == savedMode }
            if (formatMode != null) {
        ...
        (prefs["playerVodHttpProtocolMode"] ?: prefs["playerMovieHttpProtocolMode"])?.takeIf { it.isNotBlank() }?.let { savedMode ->
            val protocolMode = app.ehtudo.domain.model.VodHttpProtocolMode.entries
                .firstOrNull { entry -> entry.name == savedMode }
    ...
    private suspend fun restoreSavedLibrary(
        ...
        val categoriesByProviderId = mutableMapOf<Long, List<app.ehtudo.domain.model.Category>>()
    ...
    private fun resolveProviderIdMap(
        storedProviders: List<ProviderEntity>,
        backupProviders: List<app.ehtudo.domain.model.Provider>
    ...
    private suspend fun restorePlaybackHistory(
        history: List<app.ehtudo.domain.model.PlaybackHistory>,
        storedProviders: List<ProviderEntity>,
        backupProviders: List<app.ehtudo.domain.model.Provider>,
        conflictStrategy: BackupConflictStrategy
    ) {
...
internal fun app.ehtudo.domain.model.RecordingItem.toScheduledRecordingBackup(
    provider: app.ehtudo.domain.model.Provider,
...
internal suspend fun importScheduledRecordingBackups(
    ...
    storedProviders: List<ProviderEntity>,
    existingSchedules: MutableList<app.ehtudo.domain.model.RecordingItem>,
    conflictStrategy: BackupConflictStrategy,
    recordingManager: RecordingManager,
...
private fun app.ehtudo.domain.model.Provider.toSecureEntityForBackup(
...
private val PROVIDER_LIST_TYPE: Type = object : TypeToken<List<app.ehtudo.domain.model.Provider>>() {}.type
private val FAVORITE_LIST_TYPE: Type = object : TypeToken<List<app.ehtudo.domain.model.Favorite>>() {}.type
private val VIRTUAL_GROUP_LIST_TYPE: Type = object : TypeToken<List<app.ehtudo.domain.model.VirtualGroup>>() {}.type
private val PLAYBACK_HISTORY_LIST_TYPE: Type = object : TypeToken<List<app.ehtudo.domain.model.PlaybackHistory>>() {}.type
