...
import app.ehtudo.iptv.cast.CastConnectionState
import app.ehtudo.iptv.cast.CastManager
import app.ehtudo.iptv.cast.CastMediaRequestFactory
import app.ehtudo.iptv.cast.CastPlaybackCoordinator
import app.ehtudo.iptv.cast.CastPlaybackReportMode
import app.ehtudo.iptv.di.MainPlayerEngine
import app.ehtudo.iptv.player.LivePreviewHandoffManager
import app.ehtudo.iptv.player.LiveTranslationSession
import app.ehtudo.iptv.plugins.StreamVaultPluginManager
import app.ehtudo.iptv.util.isPlaybackComplete
...
import app.ehtudo.data.remote.stalker.StalkerUrlFactory
import app.ehtudo.data.remote.xtream.XtreamStreamUrlResolver
import app.ehtudo.data.security.CredentialDecryptionException
...
import app.ehtudo.domain.model.CombinedM3uProfileMember
...
import app.ehtudo.domain.model.Episode
import app.ehtudo.domain.model.Favorite
import app.ehtudo.domain.model.LiveChannelObservedQuality
...
import app.ehtudo.domain.model.StreamInfo
import app.ehtudo.domain.model.StreamType
import app.ehtudo.domain.model.VirtualCategoryIds
import app.ehtudo.domain.model.VideoFormat
import app.ehtudo.domain.usecase.GetCustomCategories
import app.ehtudo.domain.usecase.MarkAsWatched
import app.ehtudo.domain.usecase.ScheduleRecording
import app.ehtudo.domain.usecase.ScheduleRecordingCommand
...
import app.ehtudo.domain.repository.DownloadManager
import app.ehtudo.player.Media3PlayerEngine
import app.ehtudo.player.AUDIO_VIDEO_OFFSET_MAX_MS
import app.ehtudo.player.AUDIO_VIDEO_OFFSET_MIN_MS
import app.ehtudo.player.PlaybackState
import app.ehtudo.player.PlayerEngine
import app.ehtudo.player.PlayerError
import app.ehtudo.player.PlayerSubtitleStyle
import app.ehtudo.player.timeshift.LiveTimeshiftBackend
import app.ehtudo.player.timeshift.LiveTimeshiftState
import app.ehtudo.player.timeshift.LiveTimeshiftStatus
import app.ehtudo.player.timeshift.TimeshiftConfig
import app.ehtudo.player.playback.applyUnsafeTlsBypass
...
class PlayerViewModel @Inject constructor(
    ...
    internal val favoriteRepository: app.ehtudo.domain.repository.FavoriteRepository,
    internal val playbackHistoryRepository: PlaybackHistoryRepository,
    internal val providerRepository: app.ehtudo.domain.repository.ProviderRepository,
    internal val combinedM3uRepository: CombinedM3uRepository,
    internal val preferencesRepository: app.ehtudo.data.preferences.PreferencesRepository,
    ...
    internal val currentChannelFlow = MutableStateFlow<app.ehtudo.domain.model.Channel?>(null)
    val currentChannel: StateFlow<app.ehtudo.domain.model.Channel?> = currentChannelFlow.asStateFlow()
    ...
    internal val currentChannelFlowList = MutableStateFlow<List<app.ehtudo.domain.model.Channel>>(emptyList())
    val currentChannelList: StateFlow<List<app.ehtudo.domain.model.Channel>> = currentChannelFlowList.asStateFlow()

    internal val recentChannelsFlow = MutableStateFlow<List<app.ehtudo.domain.model.Channel>>(emptyList())
    val recentChannels: StateFlow<List<app.ehtudo.domain.model.Channel>> = recentChannelsFlow.asStateFlow()
    ...
    val remoteShortcutPreferences = preferencesRepository.remoteShortcutPreferences
        .stateIn(viewModelScope, SharingStarted.WhileSubscribed(5_000L), app.ehtudo.domain.model.RemoteShortcutPreferences())
    ...
    private var preferredSurfaceMode: app.ehtudo.domain.model.PlayerSurfaceMode =
        app.ehtudo.domain.model.PlayerSurfaceMode.AUTO
     ...
     */
    internal var channelList: List<app.ehtudo.domain.model.Channel> = emptyList()
        set(value) {
            field = value
            rebuildChannelNumberIndex()
        }
    internal var channelNumberIndex: Map<Int, app.ehtudo.domain.model.Channel> = emptyMap()
        private set
    ...
    internal fun logRepositoryFailure(operation: String, result: app.ehtudo.domain.model.Result<Unit>) {
    ...
    val playerStats: StateFlow<app.ehtudo.player.PlayerStats> =
        activeEngineState(app.ehtudo.player.PlayerStats()) { it.playerStats }
    val availableAudioTracks: StateFlow<List<app.ehtudo.player.PlayerTrack>> by lazy(LazyThreadSafetyMode.NONE) {
        activeEngineState(emptyList()) { it.availableAudioTracks }
    }
    val availableSubtitleTracks: StateFlow<List<app.ehtudo.player.PlayerTrack>> by lazy(LazyThreadSafetyMode.NONE) {
        activeEngineState(emptyList()) { it.availableSubtitleTracks }
    }
    val availableVideoQualities: StateFlow<List<app.ehtudo.player.PlayerTrack>> by lazy(LazyThreadSafetyMode.NONE) {
    ...
    internal suspend fun preparePlayer(
        streamInfo: app.ehtudo.domain.model.StreamInfo,
        requestVersion: Long,
    ...
    private suspend fun probePlaybackUrl(streamInfo: app.ehtudo.domain.model.StreamInfo): PlaybackProbeFailure? {
    ...
    private suspend fun shouldProbePlaybackUrl(url: String): Boolean {
        ...
        if (provider.type != app.ehtudo.domain.model.ProviderType.STALKER_PORTAL && cacheKey in probePassedPlaybackKeys) {
            return false
        }
        return (
            provider.type == app.ehtudo.domain.model.ProviderType.XTREAM_CODES ||
                provider.type == app.ehtudo.domain.model.ProviderType.STALKER_PORTAL
            ) &&
                    ...
                    _playerDiagnostics.update {
                        it.copy(
                            ...
                            providerSourceLabel = when (provider.type) {
                                app.ehtudo.domain.model.ProviderType.XTREAM_CODES -> "Xtream Codes"
                                app.ehtudo.domain.model.ProviderType.M3U -> "M3U Playlist"
                                app.ehtudo.domain.model.ProviderType.STALKER_PORTAL -> "Stalker/MAG Portal"
                                app.ehtudo.domain.model.ProviderType.JELLYFIN -> "Jellyfin"
                            }
                        )
                    }
                }
    ...
    internal fun updateChannelDiagnostics(channel: app.ehtudo.domain.model.Channel) {
    ...
    internal suspend fun resolvePlaybackStreamInfo(
        ...
        providerId: Long,
        contentType: ContentType
    ): app.ehtudo.domain.model.StreamInfo? {
